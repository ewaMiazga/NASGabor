...
def rasterization(
    ...
    lobe_number: Optional[int] = None,
    gmm_mode: Optional[str] = None,  # 'nasg', 'nasg_gabor', or None
    gmm_features: Optional[Tensor] = None,
    packed: bool = True,
    tile_size: int = 16,
    ...
    if gmm_mode == 'nasg':
        assert lobe_number is not None, "lobe_number must be provided when using nasg."
        # shape with 3+ dims: (C, N, ...)
        if gmm_features.ndim >= 3:
            assert gmm_features.shape[:2] == (C, N) and ((gmm_features.shape[2] - 3) % 8 == 0 or (gmm_features.shape[2] - 3) % 11 == 0), \
                f"NASG params shape invalid: {gmm_features.shape}"
        # shape with 2 dims: (N, ...)
        elif gmm_features.ndim == 2:
            assert gmm_features.shape[0] == N and ((gmm_features.shape[1] - 3) % 8 == 0 or (gmm_features.shape[1] - 3) % 11 == 0), \
                f"NASG params shape invalid: {gmm_features.shape}"
    elif gmm_mode == 'nasg_gabor':
        assert lobe_number is not None, "lobe_number must be provided when using nasg_gabor."
        # shape with 3+ dims: (C, N, ...)
        if gmm_features.ndim >= 3:
            assert gmm_features.shape[:2] == (C, N) and ((gmm_features.shape[2] - 3) % 9 == 0 or (gmm_features.shape[2] - 3) % 12 == 0), \
                f"NASG_GABOR params shape invalid: {gmm_features.shape}"
        # shape with 2 dims: (N, ...)
        elif gmm_features.ndim == 2:
            assert gmm_features.shape[0] == N and ((gmm_features.shape[1] - 3) % 9 == 0 or (gmm_features.shape[1] - 3) % 12 == 0), \
                f"NASG_GABOR params shape invalid: {gmm_features.shape}"
    else:
        if sh_degree is None and lobe_number is None:
    ...
    if sh_degree is None and lobe_number is None and gmm_mode is None:
        ...
        if gmm_mode == 'nasg':
            if gmm_features.dim() == 2:
                gmm_features = gmm_features.expand(C, -1, -1)

            # SH0 
            colors = gmm_features[..., :3]

            # 8D Features
            features = gmm_features[..., 3:]

            colors = nasg(
            lobe_number, dirs, colors, features, masks=masks, eval=eval
            )  # [C, N, 3]
        elif gmm_mode == 'nasg_gabor':
            if gmm_features.dim() == 2:
                gmm_features = gmm_features.expand(C, -1, -1)
...
def _rasterization(
    ...
    if sh_degree is None:
        ...
        colors = torch.clamp_min(colors + 0.5, 0.0)
        if gmm_mode == 'nasg':
            # Read first 3 SH channels
            if gmm_features.dim() == 2:
                gmm_features = gmm_features.expand(C, -1, -1)

            colors = gmm_features[..., :3]
            gmm_features = gmm_features[..., 3:]
            
            colors = nasg(
                lobe_number, dirs, colors, gmm_features, masks=masks, eval=eval
            )  # [C, N, 3]
