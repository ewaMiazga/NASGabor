...
# while this function is evoked, params with active_sh_degrees and max_sh_degrees are passed as coeffs.
def nasg(
    active_primitives: int, #active_sh_degree
    dirs: Tensor,  # [..., 3] # viewing direction v
    c0: Tensor,  # [3]        # base color 
    coeffs: Tensor,  # [..., max_num_primitives * 8] # coeff of the nasg 
    masks: Optional[Tensor] = None, # mask on the the whole tensor
    eval: bool = False,
) -> Tensor:
    """Computes NASG.

    Args:
        active_primitives: Number of active primitives.
        dirs: Directions. [..., 3]
        c0: Base color. [3]
        coeffs: Coefficients. [..., max_num_primitives * 8]
        masks: Optional boolean masks to skip some computation. [...,]

    Returns:
        Colors. [..., 3]
    """
    assert dirs.shape[:-1] == coeffs.shape[:-1], (dirs.shape, coeffs.shape)
    assert dirs.shape[-1] == 3, dirs.shape
    assert c0.shape[-1] == 3, c0.shape
    if eval:
        assert coeffs.shape[-1] % 11 == 0
    else:
        assert coeffs.shape[-1] % 8 == 0, coeffs.shape # nasg has 8 params 
    if masks is not None:
        assert masks.shape == dirs.shape[:-1], masks.shape
        masks = masks.contiguous()
    if eval:
        return _make_lazy_cuda_func("compute_nasg_eval_fwd")(
            active_primitives,
            dirs.contiguous(),
            c0.contiguous(),
            coeffs.contiguous(),
...
def nasg_gabor(
    ...
    dirs: Tensor,  # [..., 3] # viewing direction v
    c0: Tensor,  # [3]        # base color 
    coeffs: Tensor,  # [..., max_num_primitives * 9] (legacy) or *12 (eval)
    masks: Optional[Tensor] = None, # mask on the the whole tensor
    eval: bool = False,
) -> Tensor:
    """Computes NASG with Gabor kernels.

    Args:
        active_primitives: Number of active primitives.
        dirs: Directions. [..., 3]
        c0: Base color. [3]
        coeffs: Coefficients. [..., max_num_primitives * 9] (legacy) or *12 (eval)
        masks: Optional boolean masks to skip some computation. [...,]
        eval: If True, uses eval forward kernel with 12 params per primitive.
    ...
    assert c0.shape[-1] == 3, c0.shape
    if eval:
        assert coeffs.shape[-1] % 12 == 0, coeffs.shape
    else:
        assert coeffs.shape[-1] % 9 == 0, coeffs.shape
    if masks is not None:
        assert masks.shape == dirs.shape[:-1], masks.shape
        masks = masks.contiguous()
    if eval:
        return _make_lazy_cuda_func("compute_nasg_gabor_eval_fwd")(
            active_primitives,
            dirs.contiguous(),
            c0.contiguous(),
            coeffs.contiguous(),
...
def fully_fused_projection(
    ...
    .. note::
        ...
        In this case, a `camera_ids` tensor and `primitive_ids` tensor will be returned to indicate the
    ...
    Returns:
        ...
        - **primitive_ids**. The column indices of the projected Gaussians. Int32 tensor of shape [nnz].
...
def isect_tiles(
    ...
    if packed:
        ...
        assert primitive_ids is not None, "primitive_ids is required if packed is True"
        assert n_cameras is not None, "n_cameras is required if packed is True"
        camera_ids = camera_ids.contiguous()
        primitive_ids = primitive_ids.contiguous()
        C = n_cameras
...
class _FullyFusedProjectionPacked(torch.autograd.Function):
    ...
    def forward(
        ...
        return camera_ids, primitive_ids, radii, means2d, depths, conics, compensations
    ...
    def backward(
        ...
        if not ctx.needs_input_grad[0]:
            ...
            if sparse_grad:
                # TODO: primitive_ids is duplicated so not ideal.
                ...
                v_means = torch.sparse_coo_tensor(
                    indices=primitive_ids[None],  # [1, nnz]
...
class _NASG(torch.autograd.Function):
    """Normalized Anisotropic Spherical Gaussians"""

    @staticmethod
    def forward(
        ctx,
        active_primitives: int, # number of active_degrees
        dirs: Tensor, # v - viewing directions
        c0: Tensor,
        coeffs: Tensor, # gaussians in 3dgs-mcmc, in betas in spherical beta
        masks: Tensor
    ) -> Tensor:
        colors = _make_lazy_cuda_func("compute_nasg_fwd")(
            active_primitives, dirs, c0, coeffs, masks
        )
        ctx.save_for_backward(dirs, c0, coeffs, masks)
        ctx.active_primitives = active_primitives
        ctx.num_primitives = coeffs.shape[-1] // 8 #int representing max_sh_degree
        return colors
    
    @staticmethod
    def backward(ctx, v_colors: Tensor):
        dirs, c0, coeffs, masks = ctx.saved_tensors
        active_primitives = ctx.active_primitives
        num_primitives = ctx.num_primitives
        compute_v_dirs = ctx.needs_input_grad[1]
        v_c0, v_coeffs, v_dirs = _make_lazy_cuda_func("compute_nasg_bwd")(
            num_primitives,
            active_primitives,
            dirs,
            c0,
        ...
        if not compute_v_dirs:
            v_dirs = None
        return None, v_dirs, v_c0, v_coeffs, None


class _NASG_GABOR(torch.autograd.Function):
    """Normalized Anisotropic Spherical Gaussians with Gabor kernels"""
    ...
    def forward(
        ...
        colors = _make_lazy_cuda_func("compute_nasg_gabor_fwd")(
            active_primitives, dirs, c0, coeffs, masks
        )
        ctx.save_for_backward(dirs, c0, coeffs, masks)
        ctx.active_primitives = active_primitives
        ctx.num_primitives = coeffs.shape[-1] // 9 #int representing max_sh_degree
        return colors
    ...
    def backward(ctx, v_colors: Tensor):
        ...
        num_primitives = ctx.num_primitives
        compute_v_dirs = ctx.needs_input_grad[1]
        v_c0, v_coeffs, v_dirs = _make_lazy_cuda_func("compute_nasg_gabor_bwd")(
            num_primitives,
            active_primitives,
            dirs,
            c0,
