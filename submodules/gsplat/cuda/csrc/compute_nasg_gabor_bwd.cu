...
namespace cg = cooperative_groups;
    ...
    template <typename T>
    __global__ void compute_nasg_gabor_bwd_kernel(
        const uint32_t N,
        const uint32_t num_primitives,      // max_sh_degree
        const uint32_t active_primitives,   // active_sh_degree
        const vec3<T> *__restrict__ dirs,   // [N, 3]
        const T *__restrict__ c0,         // [3] base color
        const T *__restrict__ coeffs,       // [N, num_primitives * 9] [cosθ, cosϕ, cosτ, λ, a, k, r, g, b]
        const bool *__restrict__ masks,     // [N]
        const T *__restrict__ v_colors,     // [N, 3] - gradients w.r.t. output colors

        T *__restrict__ v_c0,            // [N, 3]
        T *__restrict__ v_coeffs,          // [N, num_primitives * 9]
        ...
        nasg_gabor_bwd(
            ...
            v_c0 + idx * 3,
            v_coeffs + idx * num_primitives * 9
        );
    }

    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> compute_nasg_gabor_bwd_tensor(
        const uint32_t num_primitives,
        const uint32_t active_primitives,
        const torch::Tensor &dirs,               // [..., 3]
        const torch::Tensor &c0,                 // [..., 3]
        const torch::Tensor &coeffs,             // [..., num_primitives * 9]
        ...
        torch::Tensor v_coeffs = torch::zeros_like(coeffs); // [..., num_primitives * 9]
        torch::Tensor v_c0 = torch::zeros_like(c0);       // [..., 3]
        torch::Tensor v_dirs;
        if (compute_v_dirs) {
            v_dirs = torch::zeros_like(dirs);             // [..., 3]
        }

        if (N) {
            compute_nasg_gabor_bwd_kernel<float>
        ...
        if (error != cudaSuccess) {
            std::cerr << "CUDA error in compute_nasg_gabor_bwd_tensor: " << cudaGetErrorString(error) << std::endl;
            throw std::runtime_error("CUDA error in compute_nasg_gabor_bwd_tensor " + std::string(cudaGetErrorString(error)));
        }
        
        return std::make_tuple(v_c0, v_coeffs, v_dirs); // [..., 3], [..., num_primitives * 9], [..., 3]
    }
} // namespace gsplat
