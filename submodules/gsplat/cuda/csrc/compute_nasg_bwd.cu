...
namespace cg = cooperative_groups;
    ...
    template <typename T>
    __global__ void compute_nasg_bwd_kernel(
        const uint32_t N,
        ...
        const T *__restrict__ coeffs,       // [N, num_primitives * 8] [x, y, z, anisotropy, lambda, r, g, b]
        const bool *__restrict__ masks,     // [N]
        const T *__restrict__ v_colors,     // [N, 3] - gradients w.r.t. output colors

        T *__restrict__ v_c0,            // [N, 3]
        T *__restrict__ v_coeffs,          // [N, num_primitives * 8]
        ...
        nasg_bwd(
            ...
            v_c0 + idx * 3,
            v_coeffs + idx * num_primitives * 8
        );
    }

    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> compute_nasg_bwd_tensor(
        const uint32_t num_primitives,
        const uint32_t active_primitives,
        const torch::Tensor &dirs,               // [..., 3]
        const torch::Tensor &c0,                 // [..., 3]
        const torch::Tensor &coeffs,             // [..., num_primitives *8]
        ...
        torch::Tensor v_coeffs = torch::zeros_like(coeffs); // [..., num_primitives * 8]
        ...
        return std::make_tuple(v_c0, v_coeffs, v_dirs); // [..., 3], [..., num_primitives * 8], [..., 3]
    }
} // namespace
