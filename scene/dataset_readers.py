...
def readColmapSceneInfo(
    ...
    print(f"Images read from {os.path.join(path, reading_dir)}")
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            image = Image.fromarray(np.array(arr * 255.0, dtype=np.ubyte), "RGB")
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    try:
        ...
        # display average color of the point cloud
        avg_color = np.mean(pcd.colors, axis=0)
        median = np.median(pcd.colors, axis=0)
        print(f"Average color of the point cloud: {avg_color}, median: {median}")
    except:
        pcd = None
