...
def training(args):
    ...
    args.scene_dnn_mean = scene.getMeanDistance()
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            # schedule lr for any other param then xyz
            if iteration > 7000:
    ...
    if args.eval:
        print(f"\nEvaluating {iteration} Performance\n")
        beta_model.load_ply(
            os.path.join(
                scene.model_path, f"point_cloud/iteration_{iteration}/point_cloud.ply"
            )
        )
        result = scene.eval()
        with open(
            os.path.join(
                scene.model_path, f"point_cloud/iteration_{iteration}/metrics.json"
            ),
            "w",
        ) as f:
